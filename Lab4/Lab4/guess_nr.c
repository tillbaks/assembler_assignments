 ...
 *  Author: staff
 * 
 * Updated by Johannes Bluml 2019-12-13
 */ 
...
static uint16_t get_nr(void)
{
	...
	while (1) {
		// Ask for number and store result in guessed_nr and length in input_length
...
static uint8_t playing_game(uint16_t rnd_nr, uint16_t *p_nr_of_guesses)
{
	...
	//UPPGIFT: �ka inneh�llet i variabeln som 'p_nr_of_guesses' pekar p�.
	(*p_nr_of_guesses)++;
